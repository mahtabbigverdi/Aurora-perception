...
class DataArguments:
    ...
    ##### NEW ###########
    ## fixing the image size for depth estimation and bounding box prediction
...
class TrainingArguments(transformers.TrainingArguments):
    ...
    ##### NEW ###########
    ## new booleans for turnning depth estimation or bounding box prediciton mode on
...
def find_all_linear_names(model):
    ...
    for name, module in model.named_modules():
        ...
        if isinstance(module, cls):
            ...
            ## remove vision encoder layers from lora tunning
...
def train(attn_implementation=None):
    ...
    if model_args.version == "v0":
     ...
     ##### NEW ##########
     ## adding new tokens to the tokenizer -- depth tokens for relative depth estimation and coordinates tokens for counting/bounding box prediction
    ...
    if training_args.depth_data or training_args.coordinates_data:
        ## change the size and dimension of the lm_head and embed_tokens weight matrices with respect to new tokenizer
        model.resize_token_embeddings(len(tokenizer))
        ## make lm_head and embed_tokens trainable
